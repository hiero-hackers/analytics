...
def main(org: str = ORG) -> None:
    ...
    # Chart titles keep the friendly name for the default org.
    title_prefix = "Hiero Hackers" if org == ORG else org
    ...
    if not language_df.empty:
        plot_bar(
            ...
            x_col="language",
            y_col="count",
            title=f"{title_prefix} — Programming Languages",
    ...
    if not activity_df.empty and activity_df["count"].sum() > 0:
        plot_pie(
            ...
            label_col="status",
            value_col="count",
            title=f"{title_prefix} — Repository Push Activity (Last 30 Days)",
