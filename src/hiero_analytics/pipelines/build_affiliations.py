...
This is a maintenance tool, not part of the default dashboard run (which just
reads the curated YAML offline). It needs network access, a ``GITHUB_TOKEN``,
and the ``gpg`` CLI:

    uv run hiero-analytics build_affiliations
...
from hiero_analytics.config.paths import EXTRA_ORGS, ORG, ORG_DATA_DIR, SRC
...
from hiero_analytics.pipelines._affiliation_entities import (
    ...
    NOREPLY,
    PERSONAL_DOMAINS,
    SMALL_ORG_FROM_COMPANY,
    SMALL_ORG_FROM_ORG,
    SMALL_ORG_NAME,
)
from hiero_analytics.pipelines._shared import shared_client

# Governance participants are drawn from the primary org plus the shared
# extra-orgs concept (config.paths.EXTRA_ORGS), so MAINTAINERS.md scanning and
# commit mining follow the same org set as the rest of the analytics.
PARTICIPANT_ORGS = tuple(dict.fromkeys((ORG, *EXTRA_ORGS)))
...
def _get_json(client: GitHubClient, url: str, params: dict | None = None) -> object | None:
    """GET a REST endpoint through the shared client; ``None`` on any failure.

    The client owns retries, backoff, and the process-wide rate limiter — this
    tool previously re-implemented all three privately. The tool's callers all
    treat ``None`` as "absent" (missing file, empty page), so every HTTP error
    maps to it.
    """
    try:
...
def email_domain(addr: str) -> str:
    """The domain part only ('@gmail.com').

    Used so a full email address is never written to disk — resolution uses the
    full address in memory; the audit logs only the domain.
    """
    addr = (addr or "").strip()
...
def org_from_email(raw: str) -> str | None:
    """Map an email's domain to a known ecosystem org (personal domains never map)."""
    if not raw or "@" not in raw:
        return None
    domain = raw.split("@")[-1].strip().lower()
    return None if domain in PERSONAL_DOMAINS else EMAIL_DOMAIN.get(domain)
...
def external_company(raw: str) -> str | None:
    ...
    low = name.lower()
    if not name or low in COMPANY_JUNK or low in COMPANY_NOT_EMPLOYER:
        return None
...
def orgs_from_membership(orgs: list[str]) -> set[str]:
    """Employer orgs implied by public GitHub org memberships (curated map only)."""
...
def org_from_md_company(raw: str) -> str | None:
    """Resolve a MAINTAINERS.md 'Company Affiliation' cell to a known org, or None."""
    norm = raw.strip().lower()
    if norm in MD_SKIP:
        return None
    if norm in MD_COMPANY_FULL:
        return MD_COMPANY_FULL[norm]
    if norm in COMPANY_FULL:
        return COMPANY_FULL[norm]
    for token in re.findall(r"[a-z0-9]+", norm):
        if token in MD_COMPANY:
            return MD_COMPANY[token]
        if token in COMPANY_TOKEN:
            return COMPANY_TOKEN[token]
    return None
...
def parse_maintainers_md(md: str):
    ...
    for line in md.splitlines():
        ...
        # Header detection must not fire on data rows: a markdown-linked handle
        # ([@alice](https://github.com/alice)) contains "github" too, and
        # re-headering mid-table silently drops rows. A real header cell names
        # the column ("GitHub ID") and never contains a github.com/ URL.
        if any("github" in c and "github.com/" not in c for c in low):
            cols = low
            continue
...
def fetch_maintainers_affiliations(client: GitHubClient) -> dict[str, str]:
    """Build ``{github_id -> declared org}`` from every repo's MAINTAINERS.md."""
    declared: dict[str, set[str]] = {}
    for org in PARTICIPANT_ORGS:
        page, repos = 1, []
        while True:
            listing = _get_json(
                client,
            ...
            if not listing:
                break
            repos += [x["name"] for x in listing]
            page += 1
        for repo in repos:
            md = None
            for fname in ("MAINTAINERS.md", "MAINTAINERS"):
                payload = _get_json(client, f"https://api.github.com/repos/{org}/{repo}/contents/{fname}")
                if payload:
                    md = base64.b64decode(payload["content"]).decode("utf-8", "replace")
                    break
            if not md:
                continue
...
def commit_author_org(client: GitHubClient, login: str) -> str | None:
    ...
    for org in PARTICIPANT_ORGS:
        payload = _get_json(
            client, "https://api.github.com/search/commits", params={"q": f"author:{login} org:{org}", "per_page": 30}
        )
        if not payload:
            continue
...
def fetch_linkedin(client: GitHubClient, login: str) -> str:
    """The login's LinkedIn URL, if linked on their profile (manual pointer only)."""
    accounts = _get_json(client, f"https://api.github.com/users/{login}/social_accounts")
    if not accounts:
        return ""
...
def resolve_participant(
    ...
    gpg_emails: list[str],
    md_org: str | None,
    fetch_commit_org: Callable[[], str | None],
) -> dict:
    """Resolve one participant's affiliation from public signals, in precedence order.

    Pure given its inputs — the GraphQL profile fields, the GPG UID emails, any
    MAINTAINERS.md declaration — plus a *lazy* commit-email resolver that is
    invoked only when nothing else places the person (it costs API calls).
    Returns the resolution fields written to the YAML and the audit CSV.

    Precedence: GPG > profile email > MAINTAINERS.md > company field > bio >
    org membership > curated small/solo domain > commit-author email.
    """
    ...
    e = org_from_email(profile["email"])
    md = md_org
    ...
    c = org_from_company(profile["company"]) or external_company(profile["company"])
    bio = org_from_bio(profile["bio"])
    org_set = orgs_from_membership(profile["orgs"])
    o = next(iter(org_set)) if len(org_set) == 1 else None

    resolved = g or e or md or c or bio or o
    small = None if resolved else derive_small_org(gpg_emails, profile["email"], profile["company"], profile["orgs"])
    resolved = resolved or small

    # Last-resort backup: mine commit-author emails only when nothing else placed them.
    commit_org = None
    if not resolved:
        commit_org = fetch_commit_org()
        resolved = commit_org

    # Obfuscated noreply addresses are not an identity signal.
    real_gpg = [a for a in gpg_emails if not a.lower().endswith("@" + NOREPLY)]
    real_email = bool(profile["email"]) and not profile["email"].lower().endswith("@" + NOREPLY)
    has_identity = bool(real_gpg) or real_email or bool(profile["company"])
    ...
    if g and resolved == g:
        decided_by = "gpg-key email"
    elif e and resolved == e:
        decided_by = "profile email"
    elif md and resolved == md:
        decided_by = "MAINTAINERS.md"
    elif c and resolved == c:
        decided_by = "company field"
    elif bio and resolved == bio:
        decided_by = "profile bio"
    elif o and resolved == o:
        decided_by = "org membership"
    elif small and resolved == small:
        decided_by = "small/solo domain"
    elif commit_org and resolved == commit_org:
        decided_by = "commit email"
    elif status == "independent":
        decided_by = "personal email only"
    elif gpg_emails or profile["email"]:
        decided_by = "obfuscated email only"
    else:
        decided_by = "no public signal"

    return {
        "gpg_emails": gpg_emails,
...
def _write_yaml(rows: list[dict], yaml_path, *, scope_of, maintainer_count: int) -> None:
    """Write the curated YAML (the source of truth), keyed by login.

    Each row carries a 'governance-role · name' comment; hand-corrected rows are
    re-flagged MANUAL with the resolver's competing guess and never overwritten.
    """
    header = (
        "# Maintainer / governance participant -> organisation, for the diversity charts.\n"
        "# Regenerate with 'hiero-analytics build_affiliations'. Seeded from public signals\n"
        ...
        f"# triage | team). Only the {maintainer_count} maintainers feed the maintainer-org charts;\n"
    ...
    logger.info("wrote %s (%d manual overrides preserved)", yaml_path, manual_count)


def _write_audit(rows: list[dict], audit_path) -> None:
    """Write the provenance audit CSV (gitignored — contains email domains)."""
...
def _fetch_profiles(client: GitHubClient, people: list[str]) -> list[dict]:
    """Fetch profile fields for every participant, batched via GraphQL aliases.

    Hand-built aliased queries (login via ``json.dumps``) rather than variables:
    GraphQL cannot parameterize an alias fan-out, and this is the one place the
    codebase batches *users* rather than repos.
    """
    rows = []
...
def main() -> None:
    """Regenerate affiliations.yaml (and the audit CSV) from public signals."""
    if not GITHUB_TOKEN:
    ...
    logger.info("manual overrides found: %d", len(manual_overrides))
    ...
    logger.info("governance participants: %d (%d maintainers)", len(people), len(maintainers))
    ...
    md_declared = fetch_maintainers_affiliations(client)
    logger.info("MAINTAINERS.md: %d handles with a resolved declared company", len(md_declared))

    rows = _fetch_profiles(client, people)

    for r in rows:
        r.update(
            resolve_participant(
                r,
                gpg_uid_emails(r["login"]),
                md_declared.get(r["login"].lower()),
                lambda login=r["login"]: commit_author_org(client, login),
            )
        )
        # Surface a LinkedIn pointer only where we still couldn't place them.
        r["linkedin"] = fetch_linkedin(client, r["login"]) if not r["resolved"] and not r["has_identity"] else ""
    ...
    logger.info(
        "affiliated %d, independent %d, unknown %d of %d",
        by_status["affiliated"],
        by_status["independent"],
        by_status["unknown"],
        len(rows),
    )

    _write_yaml(rows, yaml_path, scope_of=scope_of, maintainer_count=len(maintainers))
    _write_audit(rows, ORG_DATA_DIR / ORG / "maintainer_affiliation_audit.csv")
