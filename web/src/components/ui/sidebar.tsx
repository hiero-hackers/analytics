...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // Local change: shadcn persists this in a `sidebar_state` cookie for
      // server-rendered apps to read back. Nothing here reads it, and on
      // GitHub Pages a path=/ cookie is shared by every project on the org's
      // github.io domain — so the state is simply not persisted.
    },
    [setOpenProp, open],
  );
...
function Sidebar({
  ...
  // Local change: Radix returns focus to a DialogTrigger on close, but this
  // Sheet is opened through context (SidebarTrigger), so it has none and
  // focus fell to <body> on Escape. Remember what had focus when it opened
  // and put focus back there.
      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            {/* Local change: names this app's navigation instead of the
                generic "Sidebar" / "Displays the mobile sidebar." */}
            <SheetTitle>Sections</SheetTitle>
            <SheetDescription>Choose which part of the dashboard to show.</SheetDescription>
          </SheetHeader>
...
// Local change: shadcn's SidebarMenuButton takes a `tooltip`, shown only in
// the collapsed "icon" mode. This app uses the offcanvas mode, so the tooltip
// could never appear — yet it pulled Radix Tooltip, Popper and Floating UI
// (about 10 kB gzipped) into every page. Removed; restore it from the
// registry (`npx shadcn@latest add sidebar --diff`) if icon mode is adopted.
